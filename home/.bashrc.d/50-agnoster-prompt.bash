...
prompt_context() {
  ...
  if [[ $user != $DEFAULT_USER || -n $SSH_CLIENT ]]; then
    # TODO(kruton): make this yellow if running with privs
...
elif [[ $PROMPT_COMMAND != *\;PS1=\$\(build_prompt\)* ]]; then
  PROMPT_COMMAND="${PROMPT_COMMAND%% }" # remove trailing spaces
  PROMPT_COMMAND="${PROMPT_COMMAND%\;}" # remove trailing semi-colon
  PROMPT_COMMAND="${PROMPT_COMMAND};PS1=\$(build_prompt)"
fi
